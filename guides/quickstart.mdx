...
Lensmor's public API is designed around a typical event-intelligence workflow:
...
<Note>
  The public API uses user API keys with the `uak_` prefix. Older `sk_` examples are historical and should not be used for new integrations.
</Note>
...
The response includes subscription credits, gift credits, the total balance, and the next reset timestamp when available.
...
The event list is paginated. Store both `id` and `eventId` when possible:

- `id` is the database identifier.
- `eventId` is the public event identifier that may be easier to share across tools.
...
```bash
curl "https://platform.lensmor.com/external/events/139574/exhibitors?page=1&pageSize=20" \
  -H "Authorization: Bearer $LENSMOR_API_KEY"
```

When the response contains `semantics.accessLevel: "preview"`, read the `semantics.unlock` object before calling an unlock endpoint. It tells you whether more data is available and which action unlocks it.
...
See [Error conventions](/concepts/errors), [Credits and access](/concepts/credits-and-access), and [Rate limits](/concepts/rate-limits) for the shared contracts.
