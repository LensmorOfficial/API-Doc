...
This route is kept for compatibility. For new integrations, use [Apply recommended events (paged results)](/api-reference/profile-matching/actions-apply-recommended-events-paged).
## Endpoint
...
| `relaxed_conditions` | object, optional | Relaxed matching metadata when available. |
...
## Notes
- This route returns the same public response contract as [Apply recommended events (paged results)](/api-reference/profile-matching/actions-apply-recommended-events-paged).
