...
Use this endpoint when you want recommendation results plus the filtering and status metadata returned by the live profile-matching flow.
...
## Notes
- This route is the live implemented replacement for older PM draft references to `GET /external/profile-matching/recommendations/events`.
- Status metadata such as `status`, `profile_version`, and `active_result_version` comes from the live profile-matching paged response contract.
