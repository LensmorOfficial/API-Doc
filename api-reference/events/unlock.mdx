...
1. Find the event with [Events list](/api-reference/events/list), [Rank events](/api-reference/events/rank), or profile matching.
2. Preview exhibitors or personnel for that event.
3. Read `semantics.accessLevel` and `semantics.unlock`.
...
| `balanceAfter` | Remaining balance after the unlock operation. |
