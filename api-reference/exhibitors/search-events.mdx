...
## Notes
- This route consumes `50` credits on execution; if downstream search logic fails, the service refunds that charge.
...
- Longer queries ignore single-character company tokens and use strict token or prefix admission instead of permissive `contains` matching.
