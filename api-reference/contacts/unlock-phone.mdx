...
openapi: "POST /external/contacts/unlock-phone"
---
...
This endpoint is asynchronous. The create call returns a task identifier; t phone unlock task](/api-reference/contacts/unlock-phone-task) until the job reaches a terminal state.

<Note>
  Phone unlock is a separate workflow from email unlock. Use [Unlock contact emails](/api-reference/contacts/unlock) for email addresses.
</Note>
...
## Notes
- This endpoint creates an asynchronous unlock job. Poll [Get phone unlock task](/api-reference/contacts/unlock-phone-task) with `task_id`.
