...
`GET /external/personnel/list` accepts one or more `sourceType` values when you want the API to return people matching event-scoped source labels. Send a comma-separated value or repeat the query parameter:
...
For example, `sourceType=social,visitors` matches either supported source. The response still returns the complete non-exclusive `sourceType` array for each matched person.
