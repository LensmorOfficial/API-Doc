...
openapi: "openapi.json GET /external/personnel/events/by-linkedin"
---
...
- If you already have `personnel.id`, prefer [Personnel events](/api-reference/personnel/list-personnel-related-events) with `personnel_id`.
...
- The `personnel` object includes `linkedinActivity` and `linkedinActivityStatus` for shape consistency, but they are always `null` on this endpoint. Only [Personnel list](/api-reference/personnel/list-event-personnel) populates LinkedIn activity data.
