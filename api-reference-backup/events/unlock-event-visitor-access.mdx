...
- `200 OK` business error — free users receive body `code: 400` and `errorKey: "USER_HAS_NO_FEATURE"`; treat the action as failed.
- `400 Bad Request` — base event access is still locked, no visitor data is available, or another request input is invalid.
- `401 Unauthorized`
